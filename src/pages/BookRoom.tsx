...
import { CalendarIcon, ArrowLeft, Wifi, Coffee, Tv, Bed, Plus, Minus } from "lucide-react";
...
const BookRoom = () => {
  ...
  const bookingMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (booking) {
        booking.allocated_rooms = bookingResponse?.allocated_rooms ?? booking.rooms_allocated ?? [];
      }
  ...
  const categoryOptions = useMemo(() => {
    if (Array.isArray(availability?.categories) && availability.categories.length > 0) {
    ...
    return Array.from(roomsByCategory.entries())
      .map(([key, rooms]) => {
        const rates = rooms.map(
          (room: any) =>
            Number(room.effective_rate_per_night ?? room.price_per_night ?? 0)
        );
        const occupancies = rooms.map((room: any) => Number(room.occupancy ?? 2));
  ...
  useEffect(() => {
    setCategorySelections((prev) => {
      if (Object.keys(prev).length > 0 || categoryOptions.length === 0) return prev;
      const firstAvailable = categoryOptions.find((o: any) => (o.available_count ?? 0) > 0);
  ...
  const nights =
    checkIn && checkOut
      ? Math.max(Math.ceil((checkOut.getTime() - checkIn.getTime()) / (1000 * 60 * 60 * 24)), 1)
      : 0;
...
const estimatedTotal = roomsForSummary.reduce(
  (sum: number, room: any) =>
    sum + Number(room.effective_total ?? (Number(room.price_per_night ?? 0) * (nights || 1))),
  0
);
...
const hasCustomOverride =
  canAdjustPricing && customTotal.trim().length > 0 && Number.isFinite(parsedCustomTotal);
  ...
  const handleBooking = () => {
    ...
    if (selectedCount <= 0) {
      toast({
        ...
        description: "Choose the room classes and quantities you wish to reserve.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (roomsToBook.length === 0) {
      toast({
        ...
        description: "The selected room category is no longer available. Please pick another option.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (roomsToBook.length !== selectedCount) {
      toast({
        ...
        description: "Fewer rooms are available than you selected. Adjust the counts and try again.",
        variant: "destructive",
      });
      return;
    }
    ...
    const totalAmount = roomsToBook.reduce(
      (sum: number, room: any) =>
        sum + Number(room.effective_total ?? (Number(room.price_per_night ?? 0) * computedNights)),
      0
    );
    ...
    const categoryEntries = Object.entries(categorySelections).filter(([, count]) => count > 0);
    const categoriesPayload = categoryEntries.length ? Object.fromEntries(categoryEntries) : undefined;
    const dominantCategory = categoryEntries.length === 1 ? categoryEntries[0][0] : undefined;
    ...
    bookingMutation.mutate({
      ...
      check_in_date: format(checkIn, "yyyy-MM-dd"),
      check_out_date: format(checkOut, "yyyy-MM-dd"),
      number_of_guests: Number.parseInt(guests, 10) || fallbackCapacity || estimatedCapacity,
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="bg-card border-b">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <img src={logo} alt="Red Chilly Logo" className="w-12 h-12 object-contain" />
            <div>
              <h1 className="text-2xl font-bold">Book Your Room</h1>
              <p className="text-sm text-muted-foreground">Red Chilly The Restaurant & Banquet Hall</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl space-y-6">
        {confirmation && (
          <Card className="border-2 border-primary/40 shadow-elegant hover:shadow-elegant-selected transition-shadow">
            <CardHeader>
              <CardTitle>Reservation Confirmed</CardTitle>
              <CardDescription>Share these details at check-in.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="grid sm:grid-cols-2 gap-3 text-sm">
                <div>
                  <p className="text-muted-foreground">Booking Code</p>
                  <p className="text-lg font-semibold tracking-wide">{confirmation.booking_code}</p>
                </div>
                <div>
              ...
              {(confirmation.allocated_rooms ?? confirmation.rooms_allocated)?.length > 0 && (
                <div className="rounded-lg border bg-muted/40 p-3 space-y-2">
                  <div>
                    <p className="text-sm font-semibold">Reserved Room Categories</p>
                    <p className="text-xs text-muted-foreground">
                      Specific room numbers will be allotted by the front desk during check-in.
                    </p>
                  </div>
                  <ul className="space-y-1 text-sm">
                    {(confirmation.allocated_rooms ?? confirmation.rooms_allocated).map(
                      (room: any, index: number) => {
                        const label =
                          ...
                          room.roomClass ??
                          `Room ${index + 1}`;
                        const stayNights = room.perNight?.length ?? confirmation.nights ?? 1;
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-muted-foreground">
                              ₹{nightly.toFixed(2)} × {stayNights} night{stayNights > 1 ? "s" : ""}
                            </span>
                          </li>
                        );
                      },
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                <Button onClick={() => navigate("/")} className="gradient-primary shadow-elegant">
                  Go to Home
                </Button>
        ...
        <Card className="mb-6 shadow-elegant hover:shadow-elegant-selected transition-shadow">
          <CardHeader>
            ...
            <CardDescription>Select your hotel branch, dates, and room</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {/* Empty state */}
            {selectedOutlet && checkIn && checkOut && (!availability?.rooms || availability.rooms.length === 0) && (
            ...
            {selectedOutlet && checkIn && checkOut && availability?.rooms && availability.rooms.length > 0 && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-lg border p-4 bg-muted/40 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <p className="text-sm text-muted-foreground">Rooms selected</p>
                        <p className="text-lg font-semibold">
                          {selectedCount > 0 ? `${selectedCount} room${selectedCount === 1 ? "" : "s"}` : "None"}
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-sm">
                      ...
                      <div>
                        <p className="text-muted-foreground">Estimated Rate / Night</p>
                        <p className="font-semibold">
                          ₹{(
                            selectedCount > 0
                              ? estimatedNightlyRateTotal
                              : Number(
                                  (indicativeRate?.effective_rate_per_night ?? indicativeRate?.price_per_night) ?? 0
                                )
                          ).toFixed(2)}
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="font-semibold">₹{estimatedTotal.toFixed(2)}</p>
                      </div>
                      <div>
                    ...
                    {Object.entries(categorySelections).filter(([, count]) => count > 0).length > 0 && (
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Set how many rooms you need from each class. Availability updates automatically with your date selection.
                  </p>
                  <div className="space-y-2">
                    {categoryOptions.map((category: any) => {
                      const count = categorySelections[category.category] ?? 0;
                      const isActive = count > 0;
                      const maxForCategory = Number(category.available_count ?? 0);
                      return (
                        <div
                        ...
                        >
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div className="space-y-1">
                              <p className="font-semibold text-base">{category.category}</p>
                              <p className="text-xs text-muted-foreground">
                                Up to {category.max_occupancy || 2} guests · {category.available_count} room
                                {category.available_count === 1 ? "" : "s"} available
                              </p>
                              {category.available_count === 0 && (
                                <p className="text-xs text-destructive">Not available for the selected dates.</p>
                              )}
                            </div>
                            <div className="flex items-center gap-2">
                              <Button
                                ...
                                variant="outline"
                                size="icon"
                                onClick={() => decreaseCategory(category.category)}
                                disabled={count <= 0}
                              ...
                              <Button
                                ...
                                variant="outline"
                                size="icon"
                                onClick={() => increaseCategory(category.category)}
                          ...
                          {count >= maxForCategory && maxForCategory > 0 && (
                            <p className="mt-1 text-xs text-muted-foreground">
                              All available rooms for this category are selected.
                            </p>
                          )}

                          <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-sm">
                            <div className="text-muted-foreground">
                              From ₹{Number(category.min_rate ?? 0).toFixed(2)} per night
                            </div>
                            {category.max_rate && category.max_rate !== category.min_rate && (
                              <div className="text-muted-foreground text-xs">
                                Peaks at ₹{Number(category.max_rate).toFixed(2)}
                              </div>
                            )}
                          </div>

                          {Array.isArray(category.amenities) && category.amenities.length > 0 && (
                    ...
                    {categoryOptions.length === 0 && (
                      <div className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
                        No room categories available for the selected dates.
                      </div>
                    )}

                    {missingCategory && (
                      <p className="text-xs text-destructive">
                        {missingCategory} no longer has enough rooms for your selection. Please adjust the count.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-muted p-4 rounded-lg space-y-3">
                  ...
                  {canAdjustPricing && (
                    <div className="space-y-2">
                      <Label htmlFor="negotiated-total">Negotiated total (₹)</Label>
                      <Input
                        id="negotiated-total"
                        type="number"
                        inputMode="decimal"
                        ...
                        onChange={(event) => {
                          ...
                          setCustomTotalTouched(nextValue.trim().length > 0);
                  ...
                  <div className="flex justify-between items-center">
                    <span>{canAdjustPricing ? "Final total:" : "Total:"}</span>
                    <span className="font-semibold">₹{finalTotalDisplay.toFixed(2)}</span>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Final allocation and pricing are confirmed after booking. You will receive a detailed breakdown in your confirmation receipt.
                  </p>
                </div>
                ...
                {roomsForSummary.length > 0 && (
                  <div className="rounded-lg border p-4 space-y-2">
                    <p className="text-sm font-semibold">Your reserved rooms</p>
                    <div className="space-y-2 text-sm">
                      {roomsForSummary.map((room: any, index: number) => {
                        const categoryLabel = room.__selectedCategory ?? room.__category ?? room.room_type ?? `Room ${index + 1}`;
                        const perNight = nightlyRateForRoom(room, nights || 1);
                        const subtotal = Number(
                          room.effective_total ?? (Number(room.price_per_night ?? 0) * (nights || 1))
                        );
                        return (
                          <div key={room.id ?? index} className="flex items-center justify-between">
                            <div>
                              <span className="font-medium">{categoryLabel}</span>
                              <span className="text-muted-foreground ml-2">
                                Room {index + 1} · Occupancy: {room.occupancy ?? "2"} guests
                              </span>
                            </div>
                            <div className="text-right text-muted-foreground">
                              <div>₹{perNight.toFixed(2)}/night</div>
                              <div className="text-xs">Subtotal: ₹{subtotal.toFixed(2)}</div>
                            </div>
                          </div>
                        );
                      })}
          ...
          <CardContent>
            <div className="grid md:grid-cols-4 gap-6">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">In-room dining</p>
              </div>
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-primary mx-auto mb-3 flex items-center justify-center">
                ...
                <p className="text-sm text-muted-foreground">Smart TV in rooms</p>
              </div>
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-primary mx-auto mb-3 flex items-center justify-center">
